...
class gallery_tile extends base {
    ...
    protected $content_wrapper_template = 'block_featured_links/content_wrapper_gallery';
    protected $content_class = 'block-featured-links-content block-featured-links-content-gallery';
    protected $content_form = '\block_featured_links\tile\gallery_form_content';
    ...
    public static function get_name() {
        return get_string('multi_name', 'block_featured_links');
    }
    ...
    public function get_content_form_data() {
        ...
        if (isset($this->data->background_imgs)) {
            $data_obj->background_imgs = $this->id;
        }
        if (empty($data_obj->interval)) {
            $data_obj->interval = 4;
        }
        return $data_obj;
    }
    ...
    protected function get_content_wrapper_template_data($renderer) {
        global $PAGE;
        $PAGE->requires->js_call_amd('block_featured_links/switcher', 'resize_images', [$this->id]);
    ...
    protected function get_requirements() {
        global $PAGE;
        $PAGE->requires->js_call_amd('block_featured_links/switcher',
            'init',
            [(!isset($this->data->interval) ? $this->default_interval*1000 : $this->data->interval*1000),
                'block-featured-links-gallery-tile-'.$this->id]
        );
    }
}