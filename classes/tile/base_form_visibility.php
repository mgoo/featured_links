...
abstract class base_form_visibility extends base_form{
    ...
    public function definition () {
        ...
        $parent_content = \context_block::instance($blockid)->get_parent_context();

        if ($parent_content->contextlevel == CONTEXT_USER && $this->model->get_current_data('visibility')['visibility'] != base::VISIBILITY_CUSTOM) {
            unset($visibility_options[base::VISIBILITY_CUSTOM]);
        }
        ...
        if ($parent_content->contextlevel != CONTEXT_USER || $this->model->get_current_data('visibility')['visibility'] == base::VISIBILITY_CUSTOM) {
            ...
            if (has_capability('totara/coursecatalog:manageaudiencevisibility', \context_block::instance($blockid))) {
