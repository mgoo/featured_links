...
class block_totara_featured_links extends block_base {
    ...
    public function instance_copy($fromid) {
        ...
        foreach ($from_block_tiles as $tile) {
            ...
            $from_block_audiences = $DB->get_records('cohort_visibility', ['instanceid' => $old_tile->id, 'instancetype' => COHORT_ASSN_ITEMTYPE_FEATURED_LINKS]);
            foreach ($from_block_audiences as $from_block_audience) {
                unset($from_block_audience->id);
                $from_block_audience->instanceid = $new_tile->id;
                $DB->insert_record('cohort_visibility', $from_block_audience);
            }
        }
        return true;
    }
