 ...
 */
class block_featured_links_external_testcase extends test_helper {
    ...
    public function test_remove_tile() {
        ...
        $this->assertTrue($DB->record_exists('block_featured_links_tiles', ['id' => $tile1->id]));
        $this->assertTrue(\block_featured_links\external::remove_tile($tile1->id));
        $this->assertFalse(\block_featured_links\external::remove_tile($tile1->id));
        $this->assertFalse($DB->record_exists('block_featured_links_tiles', ['id' => $tile1->id]));
    }
    ...
    public function test_remove_tile_not_loggedin() {
        ...
        $tile1 = $this->blockgenerator->create_default_tile($instance->id);
        try {
            \block_featured_links\external::remove_tile($tile1->id);
    ...
    public function test_reorder_tiles() {
        ...
        $tile_array = [
            'block-featured-links-tile-'.$tile1->id,
            'block-featured-links-tile-'.$tile4->id,
            'block-featured-links-tile-'.$tile2->id,
            'block-featured-links-tile-'.$tile3->id];
        \block_featured_links\external::reorder_tiles($tile_array);
        $tile_array = [
            'block-featured-links-tile-'.$tile1->id,
            'block-featured-links-tile-'.$tile4->id,
            'block-featured-links-tile-'.$tile3->id,
