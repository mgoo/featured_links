 ...
 */
class block_featured_links_block_featured_links_testcase extends test_helper {
    ...
    public function test_instance_delete() {
        ...
        $this->assertEquals(2, $DB->count_records('block_featured_links_tiles', ['blockid' => $instance1->id]));
        $this->assertEquals(3, $DB->count_records('block_featured_links_tiles', ['blockid' => $instance2->id]));
        ...
        $this->assertEquals(0, $DB->count_records('block_featured_links_tiles', ['blockid' => $instance1->id]));
        $this->assertEquals(3, $DB->count_records('block_featured_links_tiles', ['blockid' => $instance2->id]));
    }
    ...
    public function test_get_tiles() {
        ...
        $block = block_instance('featured_links', $instance);
        $this->assertInstanceOf('block_featured_links', $block);

        // Make the get_tiles method accessible so that we can call it.
        $method = new \ReflectionMethod($block, 'get_tiles');
        $method->setAccessible(true);
        /* @var block_featured_links\tile\base[] $tiles */
    ...
    public function test_intance_copy() {
        ...
        $block_1 = block_instance('featured_links', $instance_1);
        $block_2 = block_instance('featured_links', $instance_2);
        ...
        $gallery_data->type = 'block_featured_links-gallery_tile';
        ...
        $default_data->type = 'block_featured_links-default_tile';
        ...
        $course_data->type = 'block_featured_links-course_tile';
        ...
        $new_tiles = $DB->get_records('block_featured_links_tiles', ['blockid' => (string)$instance_2->id]);

        $data = ['block_featured_links-gallery_tile' => $gallery_tile,
            'block_featured_links-default_tile' => $default_tile,
            'block_featured_links-course_tile' => $course_tile];
        ...
        foreach ($new_tiles as $tile) {
            ...
            $this->assertSame((string)\block_featured_links\tile\base::VISIBILITY_SHOW, $tile->visibility);
            $this->assertSame('', $tile->presetsraw);
            $this->assertSame((string)\block_featured_links\tile\base::AGGREGATION_ANY, $tile->presetsaggregation);
            $this->assertSame((string)\block_featured_links\tile\base::AGGREGATION_ANY, $tile->overallaggregation);
