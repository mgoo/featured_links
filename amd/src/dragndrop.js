...
define(['jquery', 'core/ajax', 'jqueryui'], function($, ajax){
    ...
    return {
        init: function(){
            $('.block-featured-links-layout').sortableGrid({
                cursor: 'move',
                helper: 'clone',
                placeholder: 'block-featured-links-placeholder',
                tolerance: 'intersect',
                items: ' > [id^=block-featured-links-tile-]',
                opacity: 0.5,
                update: function(event, ui) {
                    var sortedIDs = $(event.target).sortableGrid( "toArray" );
                    ajax.call([
                        {
                            methodname: 'block_featured_links_external_reorder_tiles',
                            args: {
                                tiles: sortedIDs
                            }
                        }
                ...
                start: function(event, ui) {
                    ui.item.show();
                    $('.block-featured-links-layout').sortableGrid('option',
                        'cursorAt',
